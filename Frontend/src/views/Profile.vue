      ...
      <v-col cols="12" md="8">
        <v-card class="rounded-xl shadow-lg pa-6" elevation="10">
          ...
          <div class="d-flex justify-center gap-4 mt-8">
            ...
            <v-btn prepend-icon="mdi-lock-reset" color="error" variant="outlined" @click="dialogPassword = true">
              Đổi mật khẩu
            </v-btn>
          </div>
        </v-card>
  ...
  methods: {
    ...
    async submitUpdateInfo(values) {
      ...
      try {
        ...
        this.showSnackbar(error.response?.data?.message || "Lỗi cập nhật.", "error");
      } finally {
    ...
    async submitChangePassword(values, { resetForm }) {
      ...
      try {
        ...
        this.showSnackbar("Đổi mật khẩu thành công! Vui lòng đăng nhập lại.", "success");
        ...
        this.pwdError = error.response?.data?.message || "Đổi mật khẩu thất bại.";
      } finally {
        this.isLoading = false;
      }
    },
