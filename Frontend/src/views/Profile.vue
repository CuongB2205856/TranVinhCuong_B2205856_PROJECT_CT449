      ...
      <v-col cols="12" md="10">
        <v-card class="rounded-xl shadow-lg" elevation="10">
          <v-tabs
            ...
            class="border-b"
          >
            <v-tab value="info" prepend-icon="mdi-account-box"
              >Hồ Sơ Cá Nhân</v-tab
            >
            <v-tab value="history" prepend-icon="mdi-history"
              >Lịch Sử Mượn Sách</v-tab
            >
          </v-tabs>
          ...
          <v-card-text class="pa-6">
            <v-window v-model="activeTab">
              <v-window-item value="info">
                ...
                <v-list class="mt-2" density="comfortable"> </v-list>
...
export default {
  ...
  methods: {
    ...
    async handleCancelBorrowing(item) {
      if (
        !confirm(
          `Bạn có chắc muốn hủy yêu cầu mượn sách "${item.MaSach?.TenSach}" không?`
        )
      )
        return;
      ...
      try {
        ...
        this.showSnackbar(
          error.response?.data?.message || "Lỗi khi hủy phiếu.",
          "error"
        );
      } finally {
    ...
    async handleFileUpload(event) {
      ...
      if (!["image/jpeg", "image/png", "image/jpg"].includes(file.type))
        return this.showSnackbar("Chỉ chấp nhận file ảnh.", "error");
      ...
      try {
        ...
        sessionStorage.setItem(
          "user",
          JSON.stringify({ ...currentUser, HinhAnh: imageUrl })
        );
